...
void main() {
    ...
    ShowLesterMsg("Tudo pronto para continuarmos. Passe na fábrica para dar andamento ao plano!", "Lester");
    WAIT(2000);

    while (true) {
        // Desenha marker tipo 0 (cone invertido) no local do evento enquanto não continuou
        if (!missionContinued) {
        ...
        if (!missionContinued && IsPlayerNearFactory()) {
            FloatingHelpText("Aperte B para continuar o golpe");
